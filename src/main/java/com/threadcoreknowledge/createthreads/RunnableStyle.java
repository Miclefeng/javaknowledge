...
public class RunnableStyle implements Runnable {
     ...
     * 2、使用继承 Thread 类的方式的话，那么每次想新建一个任务，只能新建立一个独立的线程，而这样做的损耗会比较大(比如重头开始创建一个线程、执行完毕以后再销毁等。如果线程的实际工作内存，也就是 run() 函数里只是简单的打印一行文字的话，那么可能线程的实际工作内容还是不如损耗来的大)。如果使用 Runnable 和 线程池，就可以大大减小这样的损耗。
